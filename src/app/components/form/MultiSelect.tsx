...
const MultiSelect: React.FC<MultiSelectProps> = ({
  ...
  useEffect(() => {
    const validSelected = defaultSelected.filter((val) =>
    ...
    <div className="w-full" ref={dropdownRef}>
      ...
      <div className="relative z-20 inline-block w-full">
        <div className="relative flex flex-col items-center">
            ...
            <div className="mb-2 flex h-11 rounded-lg border border-gray-300 py-1.5 pl-3 pr-3 shadow-theme-xs outline-none transition focus:border-brand-300 focus:shadow-focus-ring dark:border-gray-700 dark:bg-gray-900 dark:focus:border-brand-300">
              <div className="flex flex-wrap flex-auto gap-2">
                {selectedOptions.length > 0 ? (
                  ...
                  <span className="text-sm text-gray-800 dark:text-white/90 opacity-70">
                    Select option
                  </span>
                )}
              </div>
          ...
          {isOpen && (
            <div
              className="left-0 z-40 w-full overflow-y-auto bg-white rounded-lg shadow top-full max-h-select dark:bg-gray-900"
            >
