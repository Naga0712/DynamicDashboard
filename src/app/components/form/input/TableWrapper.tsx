      ...
      <div className="relative">
        <Table className="w-full table-fixed">
          <TableHeader className="sticky top-0 z-10 bg-white dark:bg-gray-900 shadow-sm border-b border-gray-100 dark:border-white/[0.05]">
            <TableRow>
              {showCheckboxes && (
                <TableCell className="px-[10px] py-3 font-medium text-gray-500 text-start" style={{ width: "48px", minWidth: "48px" }}>
                  <input
                    type="checkbox"
                  ...
                  return (
                    <TableRow key={row.id} onClick={() => onRowClick?.(row, globalIndex)}>
                      ...
                      {column.map((col) => (
                        <TableCell
                          key={col.key}
                          className="max-w-[200px] px-[10px] py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400 cursor-pointer"
                          style={col.width ? { width: col.width, minWidth: col.width } : {}}
                        >
                          <div
